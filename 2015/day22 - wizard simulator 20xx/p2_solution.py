...
def simulate(player, boss, effects, mana_spent, min_mana):
    # Hard mode rule: player loses 1 HP at the start of their turn
    player["hp"] -= 1
    if player["hp"] <= 0:
        return float("inf")  # Player loses immediately

    # Apply effects at the start of the turn
    apply_effects(player, boss, effects)

    # Check if the boss is dead after effects
    if boss["hp"] <= 0:
        return mana_spent
    ...
    for spell, properties in SPELLS.items():
        ...
        # Recurse
        if new_player["hp"] > 0:  # Only continue if player survives
